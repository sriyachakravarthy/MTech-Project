...
class TDANetBlock(nn.Module):
    def __init__(self, out_channels=128, in_channels=256, upsampling_depth=4):
        super().__init__()
...
class Recurrent(nn.Module):
    def __init__(self, out_channels=128, in_channels=256, upsampling_depth=4, _iter=5):
        super().__init__()
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 60000):
        ##changed max length after resampling eeg 
        super().__init__()
...
class neuroheed(nn.Module):
    def __init__(
        ...
        self.encoder = nn.Conv1d(
            in_channels=1,
            out_channels=256,#self.enc_num_basis, #hardcode to 256
        ...
        self.ln = GlobLN(256)#self.enc_num_basis
        ...
        mask_conv = nn.Conv1d(out_channels, 128 , 1) #self.enc_num_basis
        self.mask_net = nn.Sequential(nn.PReLU(), mask_conv)

        # Back end
        self.decoder = nn.ConvTranspose1d(
            in_channels=256,#self.enc_num_basis , #hardcode to 256
            out_channels=1,
            kernel_size=self.enc_kernel_size,
            stride=self.enc_kernel_size // 4,
            padding=self.enc_kernel_size // 2,
        ...
        self.fusion = nn.Conv1d(256+64,256, 1, bias=False)
    ...
    # Forward pass
    def forward(self, input_wav, eeg, reference=None):
        imm=input_wav.size(-1)
        #print('shape of ip',input_wav.shape)
     ...
     #   print(eeg.shape)
       # print('.............')
        ...
        #print('eeg size before concatenation',eeg.shape)
        x = torch.cat((x, eeg), 1)
        #print('size after concatenation',x.shape)
        x  = self.fusion(x)
        #print('size after fusion',x.shape)
        #exit(0)
        # Separation module
        x = self.ln(x)
     ...
     #   print('s shape',s.shape)
        x = x * s
        est_source = self.decoder(x)#.view(x.shape[0],1, x.shape[-1]))
        est_source=est_source.squeeze(1)
       # print('est after decoding',est_source.shape)
        T_origin = imm
        ...
        #     input_wav = F.pad(input_wav, (0, T_conv - T_origin))

        
        #print('est shape',est_source.shape)
        # Back end  ##THINKING 
        ...
        # if estimate shape is [B, 1, T] and source is [B, T]
       # est_source = est_source #.squeeze(1)
