...
def s4_ff_block(channels):
    return ResidualBlock(
        d_model=channels,
        layer=S4(d_model=channels, d_state=128, bidirectional=True, dropout=0.1, transposed=True),
        dropout=0.1,
    )
 
# -- the new S4-based separator --
class S4Separator(nn.Module):
    def __init__(self, N, B, num_layers=6):
        """
        ...
        self.blocks = nn.ModuleList([s4_ff_block(B) for _ in range(num_layers)])
...
class neuroheed(nn.Module):
    def __init__(self, args, N=256, L=20, B=100, H=128, K=100, R=6):
