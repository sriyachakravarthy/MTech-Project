...
import math
from bitnet import BitLinear, BitNetTransformer, BitFeedForward
import torch
import torch.nn as nn
import torch.nn.functional as F
...
class neuroheed(nn.Module):
    def __init__(self, args, N=256, L=20, B=64, H=128, K=100, R=6):
        ...
        self.encoder = S4M.encoder(self.L, self.N)
        self.separator = S4M(self.args, self.N, self.B, self.H, self.K, self.R)
        self.decoder = S4M.decoder(self.N, self.L)
