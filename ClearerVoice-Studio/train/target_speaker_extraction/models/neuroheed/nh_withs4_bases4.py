...
class S4NetBlock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.s4 = s4_block(dim)
        self.ff = ff_block(dim)
...
class Recurrent(nn.Module):
    def __init__(self, out_channels=128, in_channels=512, upsampling_depth=4, _iter=4):
        super().__init__()
        self.unet = S4NetBlock(dim=128)#TDANetBlock(out_channels, in_channels, upsampling_depth)
        self.iter = _iter
...
#     print(model)
#     print(model(audio,eeg=dummy_eeg).shape)